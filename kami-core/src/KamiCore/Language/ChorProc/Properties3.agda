...
module Chor𝔓roc/Properties3 (This : Chor𝔓roc 𝑗) where
  ...
  lift-π-single X = proj-＠ refl-≤ (proj-◻ {!πs-to-π X!})
  ...
  lift-π-impl {ps = []} Xp = proj-＠ refl-≤ (proj-◻ {!!})
  ...
  mkVar : ∀{Δ X A r ps qs} -> (¬ isNot＠ X) -> ps ≼' qs -> π X ∣ r , [] ↦ A Type -> Δ , F2-Type ps X ⊢Var A GlobalFiber[ cons (postpend qs r) ]
  mkVar {r = r} {ps} {qs} Zp [] Xp = zero done (lift-π {ps = ps} {qs = qs} {r = r} id-≼' Xp)
  mkVar {r = r} {ps} {qs} Zp (_∷_ a {bs = bs} Ps) Xp = zero (add-element {zs = (r ∷ [])} Ps ◆-≼≡ drop-post (a ∷ bs)) (lift-π {ps = ps} {qs = qs} {r = r} (a ∷ Ps) Xp)
  ...
  -- ... | no (Q , refl-≡) = zero (add-element-post Q) ( (proj-＠ refl-≤ (proj-◻ (lift-πS-direct {ps = ps} P))))
  ... | no (Q , refl-≡) = zero (add-element-post Q) ( (proj-＠ refl-≤ (proj-◻ (lift-πS-direct {ps = ps} (λ ()) P))))
  ... | yes Q with P
  -- ... | proj-＠ x₂ done = zero ((cons-post ps _) ◆-≡≼ ((Q ◆-≼ ι₀-<> {bs = (r ∷ [])}) ◆-≼≡ sym-≡ (cons-post qs _))) ( (proj-＠ refl-≤ (proj-◻ (lift-πS-direct {ps = ps} (proj-＠ refl-≤ done)))))
  ... | proj-＠ x₂ done = zero ((cons-post ps _) ◆-≡≼ ((Q ◆-≼ ι₀-<> {bs = (r ∷ [])}) ◆-≼≡ sym-≡ (cons-post qs _))) ( (proj-＠ refl-≤ (proj-◻ (lift-πS-direct {ps = ps} (λ ()) ((proj-＠ refl-≤ done))))))
  ... | proj-＠-≠ x₂ = none
